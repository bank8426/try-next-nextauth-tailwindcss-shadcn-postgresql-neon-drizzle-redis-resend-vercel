  ...
  for (let i = 1; i < 3 && page - i > 1; i++) {
    pageBefore.unshift(
      <PaginationItem key={page - i}>
        <PaginationLink href={`/library?page=${page - i}`}>
          {page - i}
        </PaginationLink>
      </PaginationItem>,
    );
      ...
      <Pagination id="pagination" className="text-light-100">
        <PaginationContent>
          {page > 1 && (
            <>
              <PaginationItem>
                <PaginationPrevious href={`/library?page=${page - 1}`} />
              </PaginationItem>
              <PaginationItem>
                <PaginationLink href={`/library?page=${1}`}>{1}</PaginationLink>
              </PaginationItem>
            </>
          )}
          ...
          {page < lastPage && (
            <>
              <PaginationItem>
                <PaginationLink href={`/library?page=${lastPage}`}>
                  {lastPage}
                </PaginationLink>
              </PaginationItem>
              <PaginationItem>
                <PaginationNext href={`/library?page=${page + 1}`} />
              </PaginationItem>
            </>
          )}
        </PaginationContent>
